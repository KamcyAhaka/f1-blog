...
<template>
  ...
  <section class="posts-highlight-section flex items-center justify-center gap-10 flex-wrap py-10">
    <CategoryCard
      v-for="(post, index) in highlightPosts"
      :key="index"
      :post="post"
      align="center"
    />
  </section>
  <section class="latest-section bg-gray-100 px-7 py-10 w-full flex flex-col gap-5">
    <h2 class="text-2xl text-center font-bold">Latest Posts</h2>
    <p class="text-sm text-center">Lorem ipsum, dolor sit amet consectetur adipisicing elit. Mollitia, adipisci.</p>
    <NuxtLayout
      name="ad-layout"
      aside-heading="Trending Posts"
      :posts="postHighlights"
    >
      <div class="main-content flex flex-col gap-5 content lg:basis-3/5">
        <div
          class="latest-post-container h-1/2 flex flex-col items-center justify-center gap-5 min-h-[300px] bg-gray-900 text-white rounded-xl px-3"
        >
          <span class="font-bold text-xs lg:text-sm">Review</span>
          <h3 class="text-center font-bold text-sm lg:text-base">2023 Monza GP Review: The Temple of Speed Delivered
          </h3>
  ...
  <section class="newest-section max-w-5xl mx-auto px-4 py-5 flex flex-col items-start gap-5">
    <h2 class="text-xl font-bold flex gap-2 relative">
      Newest from
      <button
        @click="showNewestCategoriesDropdown = !showNewestCategoriesDropdown"
        class="categories-toggle flex gap-3 items-center"
      >
      ...
      <CategoryDropdown
        v-if="showNewestCategoriesDropdown"
        :categories="postCategories"
      />
    </h2>
    <div class="newest-post-container grid gap-5 grid-cols-1 mx-auto md:px-5 md:grid-cols-2 lg:grid-cols-4">
      <CategoryCard
        v-for="(post, index) in newestPosts"
        :key="index"
        :post="post"
      />
    </div>
